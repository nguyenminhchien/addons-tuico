...
class tco_project(models.Model):
    ...
    clm_cmpNew = fields.Boolean('New Compound', default=True, tracking=True)
    clm_Updatedate = fields.Datetime('Modify Date', default=fields.Datetime.now)
    clm_compound_id = fields.Many2one('tco.compound', 'Compound', required=True, tracking=True)
    # clm_tags = fields.Many2many(string='Tags')
    clm_descriptionVN = fields.Char('Notes')
    ...
    clm_MaterialCompound = fields.Many2one('product.product', string='Material')
    ...
    def name_get(self):
        ...
        for res in self:
            # strcmp1 = ""
            # strcmp2 = ""
            # if res.clm_compound_id:
            #     strcmp1 = "["
            #     strcmp2 = "]"
            # name = strcmp1 + (res.clm_compound_id.name or '') + strcmp2 + (res.name or '')
            name = res.name
    ...
    def _action_open_all_projects(self):
        action = self.env['ir.actions.act_window']._for_xml_id(
            # 'tco_experiment.open_view_tco_project_all' if not self.user_has_groups('tco_experiment.tco_group_tco_project_stages') else
